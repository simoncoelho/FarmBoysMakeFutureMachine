...
void go_straight_check_intersection(void) {
	...
	while (leftBits + rightBits < 410) {
		...
		if (abs(controlSignal) < 10) {
			leftSpeed = TRAVELSPEED + 30 + (CAMERA_ORIENTATION * (-1))*(controlSignal);  // 2018 Camera Orientation: Connector at Front
			rightSpeed = TRAVELSPEED + 30 + (CAMERA_ORIENTATION * (+1))* (controlSignal); // Adjust the left and right wheel speeds
		}
		else {
			leftSpeed = TRAVELSPEED + 30 + (CAMERA_ORIENTATION * (-1))*controlSignal;  // 2018 Camera Orientation: Connector at Front
			rightSpeed = TRAVELSPEED + 30 + (CAMERA_ORIENTATION * (+1))* controlSignal; // Adjust the left and right wheel speeds
