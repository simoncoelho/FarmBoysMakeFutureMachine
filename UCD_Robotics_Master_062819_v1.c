...
static volatile int binaryImage[4], derError[2], intError, IR_PLANT, IR_PIN, BOARD_DIRECTION, COLOR, MASTER_COLOR, IR_PLANT_DROP, OFF_SPEED = 50;
...
void drop_plant(int Cwise_or_CCwise) {
	// Insert code here
	...
	rotate_robot_half_turn(Cwise_or_CCwise*(-1));
	...
	drive_speed(leftSpeed, rightSpeed);
	pause(1300);
	rotate_robot_half_turn(Cwise_or_CCwise);
...
void rotate_robot_half_turn(int Cwise_or_CCwise) {
	// Turns robot 90 deg to left or right
 	...
 	pause(turn_speed * 875.0);  // Slightly increased the turning time because it wasnt rotating fully
...
void go_straight_check_IR(void) {
	...
	while (IR_PLANT < 1) {
		...
		controlSignal = (int)(KP_HEADING * ((float)posError[4]) + (intError*KI_HEADING) + (derError[0] * KD_HEADING)); // posError[4] contains the most recent position error
																													   ...
																													   // if integral or derivative control is desired
		if (controlSignal > 20) controlSignal = 15; // limit the control signal to avoid wild overcorrections
		if (controlSignal < -20) controlSignal = -15;
...
////////////////////////////////////////
			...
			if (MASTER_COLOR > 0){
				...
				NAV_PLANT = CURRENT_PLANT -1;
				milliseconds = 0;
				while(milliseconds < (NAV_PLANT * 300)){
					// navigation loop goes here
					controlSignal = (int)(KP_HEADING * ((float)posError[4]) + (intError*KI_HEADING) + (derError[0] * KD_HEADING)); // posError[4] contains the most recent position error
