...
static volatile int binaryImage[4], derError[2], intError, IR_PLANT, IR_PIN, BOARD_DIRECTION, COLOR, MASTER_COLOR, IR_PLANT_DROP, OFF_SPEED = 30, CURRENT_PLANT;
...
void go_straight_check_intersection(void) {
	...
	while( milliseconds < 1000){
		while (leftBits + rightBits < 410) {
			...
			if (abs(controlSignal) < 10) {
				leftSpeed = TRAVELSPEED + OFF_SPEED*milliseconds/1000 + (CAMERA_ORIENTATION * (-1))*(controlSignal);  // 2018 Camera Orientation: Connector at Front
				rightSpeed = TRAVELSPEED + OFF_SPEED*milliseconds/1000 + (CAMERA_ORIENTATION * (+1))* (controlSignal); // Adjust the left and right wheel speeds
			}
			else {
				leftSpeed = TRAVELSPEED + OFF_SPEED*milliseconds/1000 + (CAMERA_ORIENTATION * (-1))*controlSignal;  // 2018 Camera Orientation: Connector at Front
				rightSpeed = TRAVELSPEED + OFF_SPEED*milliseconds/1000 + (CAMERA_ORIENTATION * (+1))* controlSignal; // Adjust the left and right wheel speeds
	...
	while (leftBits + rightBits < 410) {
		...
		if (abs(controlSignal) < 10) {
			leftSpeed = TRAVELSPEED + OFF_SPEED + (CAMERA_ORIENTATION * (-1))*(controlSignal);  // 2018 Camera Orientation: Connector at Front
			rightSpeed = TRAVELSPEED + OFF_SPEED + (CAMERA_ORIENTATION * (+1))* (controlSignal); // Adjust the left and right wheel speeds
		}
		else {
			leftSpeed = TRAVELSPEED + OFF_SPEED + (CAMERA_ORIENTATION * (-1))*controlSignal;  // 2018 Camera Orientation: Connector at Front
			rightSpeed = TRAVELSPEED + OFF_SPEED + (CAMERA_ORIENTATION * (+1))* controlSignal; // Adjust the left and right wheel speeds
...
void drop_sequence(void) {
	...
	if(CURRENT_PLANT < 5){
		if (MASTER_COLOR > -1){
			...
			while(milliseconds < (NAV_PLANT * 800)){
				// navigation loop goes here
				controlSignal = (int)(KP_HEADING * ((float)posError[4]) + (intError*KI_HEADING) + (derError[0] * KD_HEADING)); // posError[4] contains the most recent position error
		...
		if (MASTER_COLOR < 0) {
			...
			rotate_robot(BOARD_DIRECTION*(-1));
			NAV_PLANT = 6 - CURRENT_PLANT;
			milliseconds = 0;
			while(milliseconds < (NAV_PLANT * 500)){
				// navigation loop goes here
				controlSignal = (int)(KP_HEADING * ((float)posError[4]) + (intError*KI_HEADING) + (derError[0] * KD_HEADING)); // posError[4] contains the most recent position error
...
void rotate_robot(int Cwise_or_CCwise) {
	...
	pause(turn_speed * 1200.0);  // Slightly increased the turning time because it wasnt rotating fully
...
void rotate_robot_half_turn(int Cwise_or_CCwise) {
	// Turns robot 90 deg to left or right
 	...
 	pause(turn_speed * 750.0);  // Slightly increased the turning time because it wasnt rotating fully
...
void go_straight_check_IR(void) {
	...
	while (IR_PLANT < 1) {
		...
		if (abs(controlSignal) < 10) {
			...
			leftSpeed = TRAVELSPEED + OFF_SPEED + 20 + (CAMERA_ORIENTATION * (-1))*controlSignal;  // 2018 Camera Orientation: Connector at Front
			rightSpeed = TRAVELSPEED + OFF_SPEED + 20 + (CAMERA_ORIENTATION * (+1))* controlSignal; // Adjust the left and right wheel speeds
		}
...
void go_to_line(void) {
	...
	while (leftBits + rightBits < 410) {
		// navigation loop goes here
		leftSpeed = TRAVELSPEED + OFF_SPEED;
		rightSpeed = TRAVELSPEED + OFF_SPEED;
		drive_speed(leftSpeed, rightSpeed);
...
void go_to_line_after_drop(void) {
...
void rotate_robot_backwards(int Cwise_or_CCwise){
	...
	pause(turn_speed * 1450.0);  // Slightly increased the turning time because it wasnt rotating fully
...
void rotate_robot_forwards(int Cwise_or_CCwise) {
	...
	pause(turn_speed * 1050.0);  // Slightly increased the turning time because it wasnt rotating fully
...
void go_to_line_check_color(void) {
	...
	*/
	...
	while (1) {
		...
		while(CURRENT_PLANT < 9){
			...
			go_to_line_check_color();

			drop_sequence();

			rotate_robot_half_turn(BOARD_DIRECTION*-1);
