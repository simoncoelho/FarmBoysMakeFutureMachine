...
static volatile int binaryImage[4], derError[2], intError, IR_PLANT, BOARD_DIRECTION;
...
int set_board_direction(void) {
    float IR_RIGHT_VOLTS; //right IR sensor
    float IR_LEFT_VOLTS; //left IR sensor
    int counter;
    ...
    while(counter > 0 ){ // take 10 different point and use the sum for which to use
        ...
        IR_RIGHT_VOLTS = IR_RIGHT_VOLTS + adc_volts(IR_PIN_RIGHT);
        IR_LEFT_VOLTS = IR_LEFT_VOLTS + adc_volts(IR_PIN_LEFT);
        counter = counter - 1;
        pause(55);
...
void rotate_robot(int Cwise_or_CCwise) {
	...
	pause(turn_speed * 1275.0);  // Slightly increased the turning time because it wasnt rotating fully
...
int check_IR_distance( int BOARD_DIRECTION) {

    int IR_PIN; //pin that will be read from
    int counter; // variable to loop over the output from IR
    ...
    if (IR_RIGHT_VOLTS > IR_LEFT_VOLTS) {  // larger voltage indicates a closer dist.
        IR_PIN = IR_PIN_RIGHT; //right side of the board
    } else {
        IR_PIN = IR_PIN_LEFT; //left side of the board
    }
    
	float IR_volts;
	while (1) {

		IR_volts = adc_volts(IR_PIN);
...
void go_backwards_check_IR(void) {
    // Insert code here
    // navigation loop goes here
    ...
    while (delaycounter > 0) {
        ...
        /*leftSpeed = TRAVELSPEED + ( CAMERA_ORIENTATION * (-1))*controlSignal;  // 2018 Camera Orientation: Connector at Front
         rightSpeed = TRAVELSPEED + ( CAMERA_ORIENTATION * (+1))* controlSignal; // Adjust the left and right wheel speeds */
        drive_speed(-1*leftSpeed, -1*rightSpeed); // set the drive_speed based upon the control signal
        delaycounter = delaycounter - 1;
  ...
  /*
	...
	*/
	...
	while (1) {
        ...
        BOARD_DIRECTION = set_board_direction();

        print("%d", BOARD_DIRECTION);
